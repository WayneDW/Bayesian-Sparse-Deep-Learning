...
parser.add_argument('-data', default='boston', type=str, help='dataset: boston/yacht/energy-efficiency/wine/concrete')
""" Sensible hyparameters """
...
parser.add_argument('-wdecay', default=1e-4, type=float, help='L2 penalty')
...
parser.add_argument('-cuda', default=0, type=int, help='not use cuda by default')
...
net = BayesNet(cols, pars.hidden, total_samples, criterion, pars)
...
for epoch in range(pars.total_epochs):
    ...
    for i in range(total_batches):
        ...
        if pars.c in ['sa', 'em', 'sghmc']:
            """ Simulated annealing """
            sampler.invT = sampler.invT * pars.anneal
