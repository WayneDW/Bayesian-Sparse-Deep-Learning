...
class BayesNet(Net):
    def __init__(self, features, hidden_node, total_samples, pars):
        ...
        self.sd, self.l2 = pars.sd, pars.l2
        ...
        for name, param in self.named_parameters():
            if name.endswith('weight'):
                ...
                self.d_star[name] = torch.FloatTensor(param.data.size()).fill_(pars.l2)
                self.d_star0[name] = torch.FloatTensor(param.data.size()).fill_(pars.l2)
                self.d_star1[name] = torch.FloatTensor(param.data.size()).fill_(pars.l2)
                self.theta[name] = 0.5
                self.mask[name] = param.data < -1e8
                self.num_pars += np.prod(param.data.size())

    def cal_nlpos(self, outputs, labels, criterion):
        nlloss = criterion(outputs, torch.unsqueeze(labels, dim=1)) * self.N / self.batch
        self.likelihood = nlloss.item()
        for name, param in self.named_parameters():
            if self.c == 'sghmc':
                nlloss += 0.5 * self.l2 * param.norm(2)**2
            else:
                if name.endswith('weight'):
